...
class API
{
    ...
    {
        if ($this->settings['run_type'] == 1 AND $this->settings['hook_reply'] AND !$this->webhookreply_used AND !in_array($method, $this->WH_BL)) {
        ...
        $this->processError($method, $response);
        if ($json) return json_decode($response);
        else return $response;
    }

    public function processError($method, $response)
    {
    ...
    public function trace($msg)
    {
        if ($this->settings['debug_mode']) {
            if (substr(PHP_OS, 0, 3) == 'WIN') {
                if (!is_string($msg)) {
                    $msg = unserialize(iconv('UTF-8', 'CP866', serialize($msg)));
                } else $msg = iconv('UTF-8', 'CP866', $msg);
            }
            print_r($msg);
            print_r(PHP_EOL);

        }
        if ($this->settings['log_mode']) {
