...
class Calendar
{
    ...
    public function build($time = null)
    {
        ...
        if ($this->allMonth) {
            ...
            /*$navButtons = [];
            $prev = strtotime('-1 month', $today);
            if($this->compareWithIgnoreList($prev)) $navButtons[] = Keyboard::btn('<', 'calendar.m/'.$prev);
            else $navButtons[] = Keyboard::btn(' ', 'calendar.ignore');
            $navButtons[] = Keyboard::btn(' ', 'calendar.ignore');
            $next = strtotime('+1 month', $today);
            if($this->compareWithIgnoreList($next)) $navButtons[] = Keyboard::btn('>', 'calendar.m/'.$next);
            else $navButtons[] = Keyboard::btn(' ', 'calendar.ignore');
            $this->menuHandler->arrayRow($navButtons);*/
        }
